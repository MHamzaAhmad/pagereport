import { zValidator } from "@hono/zod-validator";
import { Hono } from "hono";
import { createReportInput } from "@/domain/api";
import { NothingToUnlockError, ReportNotFoundError } from "@/services";
import type { AppEnv } from "@/transport/http/types";
import { reportQuotaMiddleware } from "@/transport/middleware/report-quota";
import { turnstileMiddleware } from "@/transport/middleware/turnstile";

export const reportsRouter = new Hono<AppEnv>()
	.post(
		"/",
		turnstileMiddleware,
		reportQuotaMiddleware,
		zValidator("json", createReportInput),
		async (c) => {
			const input = c.req.valid("json");
			const report = await c.var.container.services.reports.create(input);
			return c.json(report, 201);
		},
	)
	.get("/:id", async (c) => {
		const id = c.req.param("id");
		const report = await c.var.container.services.reports.getById(id);
		if (!report) {
			return c.json({ error: "not found" }, 404);
		}
		return c.json(report);
	})
	.get("/:id/modules/:moduleType", async (c) => {
		const reportId = c.req.param("id");
		const moduleType = c.req.param("moduleType");
		const { modules } = c.var.container.services;
		const run = await modules.getByReportAndType(reportId, moduleType);
		if (!run) {
			return c.json({ error: "not found" }, 404);
		}
		return c.json(modules.toResponse(run));
	})
	.post("/:id/purchase", async (c) => {
		const reportId = c.req.param("id");
		const { purchases } = c.var.container.services;
		try {
			const result = await purchases.createCheckout(reportId);
			return c.json(result, 201);
		} catch (err) {
			if (err instanceof ReportNotFoundError) {
				return c.json({ error: "report not found" }, 404);
			}
			if (err instanceof NothingToUnlockError) {
				return c.json({ error: "nothing to unlock" }, 400);
			}
			throw err;
		}
	});
